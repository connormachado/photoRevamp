...
- **A `CollapsiblePanel`-docked rail clips on BOTH axes, not just the animating width.** Its width-animating wrapper (`CollapsiblePanel.jsx`) sets a single `overflow: hidden`, so a `position: absolute` popover anywhere inside a narrow rail (e.g. `ToolRail`'s Clear-all confirm) gets silently clipped left/right/above/below the moment it extends past that panel's own box — `VerdictButtons`' confirm dialog only avoids this because it matches its own (280px-wide) panel's width instead of overflowing past it. A popover that needs to be wider than its rail should use `position: fixed`, anchored off the trigger's own `getBoundingClientRect()` — no ancestor here sets a `transform`/`filter`, so `fixed` escapes the clip and resolves against the viewport while staying a real DOM descendant of the trigger's wrapper for outside-click detection.
- **A `SyncedPanels.jsx` synced-panel collapse is NOT `CollapsiblePanel`, and don't reroute it there.** `CollapsiblePanel.jsx` assumes a fixed-pixel sidebar docked at a true screen edge; each of the three synced panels (Original / Removed·timelapse / Trimmed result) is a `flex:1` peer whose width comes from splitting the row, with height driven by the loaded video's `aspectRatio` — no stable pixel width to hand a dock animation. The Removed·timelapse panel (kept in the visual middle on purpose) collapses by having the caller stop rendering that `Panel` entirely, not by shrinking it to a strip; a small `<>` button occupies its exact flex slot to re-expand, so Original/Trimmed (`flex:1`) reflow to fill the row the same way any flex sibling's removal would.
